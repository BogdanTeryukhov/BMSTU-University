...
void priority_queue_init(PriorityQueue *q, int capacity)// конструктор очереди с приоритетами
{
    q->capacity = capacity;
    q->count = 0;
...
PriorityQueueUnit priority_queue_pop(PriorityQueue *q)// удаление элемента
{
    PriorityQueueUnit x = q->heap[0];
    q->heap[0].priority = MIN;
    q->count = priority_queue_sift_down(q->heap, q->capacity);
    return x;
}

void priority_queue_insert(PriorityQueue *q, PriorityQueueUnit value)// вставка элемента
{
    int i = q->count;
    q->heap[i] = value;
