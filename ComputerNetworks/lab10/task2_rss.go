...
func clear() {
	...
	db.Query("TRUNCATE iu9networkslabs.iu9Makezhlab10")
}
...
func save() {
	...
	for _, item := range feed.Items {
		...
		rows, err := db.Query("select * from iu9networkslabs.iu9Teryukhovlab10 where title = ? and link = ? and description = ?"+
		...
		if !rows.Next() {
			_, err = db.Exec("insert into iu9networkslabs.iu9Teryukhovlab10 (title, link, pubdate, description, categories, authors)"+
